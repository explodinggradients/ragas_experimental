...
# %% auto 0
__all__ = ['DEFAULT_COLUMN_SETTINGS', 'COLOR_MAP', 'ColumnType', 'FieldMeta', 'Number', 'Text', 'get_colors_for_options',
           'Select', 'MultiSelect', 'Checkbox', 'Date', 'Custom', 'ModelConverter']
...
class ModelConverter:
    ...
    def infer_field_type(annotation, field_info=None):
        ...
        if origin is t.Annotated and len(args) > 1:
            base_type = args[0]
            base_origin = t.get_origin(base_type)
            base_args = t.get_args(base_type)
            ...
            if field_meta is not None:
                if isinstance(field_meta, Select) and base_origin is t.Literal:
                    # Special handling for Literal types with Select metadata
                    literal_values = list(base_args)

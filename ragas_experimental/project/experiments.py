...
# %% auto 0
__all__ = ['memorable_names', 'create_experiment_columns', 'version_experiment', 'ExperimentProtocol']
...
def version_experiment(
    ...
    if repo.is_dirty(untracked_files=True):
        ...
        print("No changes detected. Using current HEAD.")
    ...
    if create_branch:
        ...
        if version_name in [b.name for b in repo.branches]:
            print(f"Branch {version_name} already exists, updating.")
            branch = repo.branches[version_name]
            branch.checkout()
            # Force update the branch to point to current commit
    ...
    if create_tag:
        ...
        if version_name in repo.tags:
            # Delete existing tag
            repo.delete_tag(version_name)
        
        # Create tag
        tag = repo.create_tag(version_name, ref=commit_hash, 
                             message=f"Experiment version for {experiment_name}")
        print(f"Created tag: {version_name}")
